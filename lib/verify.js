...
function logcompare(input,output,shouldbe){
    ...
    console.log("-------------------");
    console.log("Output should be: " + JSON.stringify(shouldbe, null, 2).replace(/\\n/g,"\n"));
    console.log("-------------------");
}
    ...
    exercise.addProcessor(function (mode, callback) {
        ...
        if(mode === "run"){
            ...
            try {
                ...
                if(!_.isEqual(result.replace(/\r?\n|\r/g,""),run.expect.replace(/\r?\n|\r/g,""))){
                    self.emit('fail','Test');
                    passed = false;
                }else{
                    self.emit('pass','Test');
            ...
            _(tests).forEach(function(item,element){
                ...
                try {
                    ...
                    var result = _.isEqual(item.shouldbe.replace(/\r?\n|\r/g,""),usersolution(item.input).replace(/\r?\n|\r/g,""));
